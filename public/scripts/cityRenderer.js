/**
 * @file cityRenderer.js - Renderizador do mapa da cidade (Goose Cape City)
 * @description Carrega o JSON convertido do TMX e renderiza tile layers + objects.
 * Usa offscreen canvas cache para as tile layers (performance).
 * @module CityRenderer
 */
...
export async function loadCityAssets() {
    ...
    try {
        ...
        logger.warn('[CityRenderer] Falha ao carregar JSON do mapa: ' + e.message);
        assetsLoading = false;
        return;
    }
        ...
        if (ts.image) {
            imagePromises.push(
                loadImg(`city_map/${ts.image}`).then(img => {
                    ...
                    else logger.warn(`[CityRenderer] Imagem não encontrada: ${ts.image}`);
                })
            );
        }
            ...
            for (const tile of ts.tiles) {
                ...
                imagePromises.push(
                    loadImg(`city_map/${tile.image}`).then(img => {
                        ...
                        else logger.warn(`[CityRenderer] Imagem não encontrada: ${tile.image}`);
                    })
                );
            }
        }
    ...
    logger.info('[CityRenderer] Assets da cidade carregados');
}
...
function buildBackgroundCache() {
    ...
    _bgCache = offCanvas;
    logger.info('[CityRenderer] Background cache construído');
}
...
export function getCityObjects() {
    ...
    // Itera sobre todas as camadas do mapa (preservando ordem das objectgroups)
    for (const layer of mapData.layers) {
